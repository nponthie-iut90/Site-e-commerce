...
def client_coordonnee_edit_valide():
    ...
    sql = '''SELECT * FROM utilisateur WHERE id_utilisateur = %s'''
    mycursor.execute(sql, (id_client,))
    _utilisateur = mycursor.fetchone()
    get_db().commit()

    # Vérifier si un utilisateur avec le même e-mail ou le même login existe déjà
    sql_check_login = '''SELECT * FROM utilisateur WHERE login = %s AND id_utilisateur != %s'''
    mycursor.execute(sql_check_login, (login, id_client,))
    check_login = mycursor.fetchone()
    sql_check_email = '''SELECT * FROM utilisateur WHERE email = %s AND id_utilisateur != %s'''
    mycursor.execute(sql_check_email, (email, id_client,))
    ...
    if check_login or check_email:
        ...
        qsl = '''UPDATE utilisateur SET login = %s, nom = %s, email = %s
        WHERE id_utilisateur = %s'''
    mycursor.execute(qsl, (login, nom, email, id_client,))
...
def client_coordonnee_add_adresse_valide():
    ...
    sql = '''INSERT INTO adresse(nom, rue, code_postal, ville, date_utilisation, utilisateur_id)
    VALUES (%s, %s, %s, %s, DATE(NOW()), %s)'''
...
def client_coordonnee_edit_adresse_valide():
    ...
    id_adresse = request.form.get('id_adresse')
    sql = '''UPDATE adresse
    SET nom = %s, rue = %s, code_postal = %s, ville = %s, utilisateur_id = %s
    WHERE id_adresse = %s AND utilisateur_id = %s'''
    mycursor.execute(sql, (nom, rue, code_postal, ville, id_client, id_adresse, id_client))
    get_db().commit()
