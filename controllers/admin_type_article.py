...
def show_type_article():
    ...
    sql = '''SELECT type_ski.id_type_ski, type_ski.libelle_type_ski, COUNT(ski.id_ski) AS nbr_articles
FROM type_ski
LEFT JOIN ski ON type_ski.id_type_ski = ski.type_ski_id
GROUP BY type_ski.id_type_ski, type_ski.libelle_type_ski;
'''
...
def edit_type_article():
    id_type_article = request.args.get('id_type_article', '')
    mycursor = get_db().cursor()
    sql = '''SELECT * FROM type_ski WHERE id_type_ski = %s'''
    mycursor.execute(sql, (id_type_article,))
